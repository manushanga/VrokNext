...
int resampler_latency(void *_r)
{
	resampler * r = ( resampler * ) _r;
	return resampler_input_delay(r);
}
...
void resampler_write_sample(void *_r, short s)
{
    ...
    resampler_write_sample_float(_r, s32);
}
...
static int resampler_run_blep(resampler * r, float ** out_, float * out_end)
{
    ...
    if ( in_size > 0 )
    {
        ...
        do
        {
            ...
            inv_phase = FM_fractionalf(inv_phase); //FM_fmodf(inv_phase, 1.0f);
        }
        while ( in < in_end );
    ...
    if ( in_size > 0 )
    {
        ...
        do
        {
            ...
            *out++ = sample;
            ...
            phase = FM_fractionalf(phase); //FM_fmodf(phase, 1.0f);
        }
        while ( in < in_end );
...
static int resampler_run_sinc(resampler * r, float ** out_, float * out_end)
{
    ...
    if ( in_size > 0 )
    {
        ...
        do
        {
            ...
            kernel_sum = FM_reciprocalf(kernel_sum); //1.0 / kernel_sum;
